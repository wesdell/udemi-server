...
import { Course, UserCourseProgress } from "../models";
...
export const getUserCourseProgress = async (
  ...
  try {
    const progress = await UserCourseProgress.get({ userId, courseId } as { userId: string, courseId: string });
    if (!progress) {
      res
        .status(404)
...
export const updateUserCourseProgress = async (
  ...
  try {
    let progress = await UserCourseProgress.get({ userId, courseId } as { userId: string, courseId: string });
